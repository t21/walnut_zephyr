...
static int bm280_reg_read(struct bmp280_data *data,
			  u8_t start, u8_t *buf, int size)
{
#ifdef CONFIG_BMP280_DEV_TYPE_I2C
	return i2c_burst_read(data->i2c_wrap, data->i2c_slave_addr,
			      start, buf, size);
#elif defined CONFIG_BMP280_DEV_TYPE_SPI
	u8_t addr;
...
static int bm280_reg_write(struct bmp280_data *data, u8_t reg, u8_t val)
{
#ifdef CONFIG_BMP280_DEV_TYPE_I2C
	return i2c_reg_write_byte(data->i2c_wrap, data->i2c_slave_addr,
				  reg, val);
#elif defined CONFIG_BMP280_DEV_TYPE_SPI
 ...
 */
static void bmp280_compensate_temp(struct bmp280_data *data, s32_t adc_temp)
{
	s32_t var1, var2;
...
static void bmp280_compensate_press(struct bmp280_data *data, s32_t adc_press)
{
	s64_t var1, var2, p;
...
static int bmp280_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	// 	size = 8;
	// }
	ret = bm280_reg_read(data, BMP280_REG_PRESS_MSB, buf, size);
	if (ret < 0) {
		return ret;
	}
...
static int bmp280_read_compensation(struct bmp280_data *data)
{
	u16_t buf[12];
	// u8_t hbuf[7];
	int err = 0;

	err = bm280_reg_read(data, BMP280_REG_COMP_START,
			     (u8_t *)buf, sizeof(buf));
...
static int bmp280_chip_init(struct device *dev)
{
	...
	err = bm280_reg_read(data, BMP280_REG_ID, &data->chip_id, 1);
	if (err < 0) {
		return err;
	}
	...
	err = bmp280_read_compensation(data);
	if (err < 0) {
		return err;
	}
	...
	err = bm280_reg_write(data, BMP280_REG_CTRL_MEAS, BMP280_CTRL_MEAS_VAL);
	if (err < 0) {
		return err;
	}

	err = bm280_reg_write(data, BMP280_REG_CONFIG, BMP280_CONFIG_VAL);
	if (err < 0) {
		return err;
	}
...
#ifdef CONFIG_BMP280_DEV_TYPE_SPI
static inline int bmp280_spi_init(struct bmp280_data *data)
{
	data->spi = device_get_binding(CONFIG_BMP280_SPI_DEV_NAME);
	if (!data->spi) {
		SYS_LOG_DBG("spi device not found: %s",
			    CONFIG_BMP280_SPI_DEV_NAME);
		return -EINVAL;
	}
	...
	data->spi_cfg.slave = CONFIG_BMP280_SPI_DEV_SLAVE;
...
int bmp280_init(struct device *dev)
{
    ...
    SYS_LOG_INF("Init BMP280");

#ifdef CONFIG_BMP280_DEV_TYPE_I2C
	data->i2c_wrap = device_get_binding(CONFIG_BMP280_I2C_MASTER_DEV_NAME);
	if (!data->i2c_wrap) {
	...
	data->i2c_slave_addr = BMP280_I2C_ADDR;
#elif defined CONFIG_BMP280_DEV_TYPE_SPI
	if (bmp280_spi_init(data) < 0) {
...
DEVICE_AND_API_INIT(bmp280, CONFIG_BMP280_DEV_NAME, bmp280_init, &bmp280_data,
